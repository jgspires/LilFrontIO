...
export class NationStructureBehavior {
  ...
  private buildReachableStations(): Array<{
    ...
    const maxTradeGold = Math.max(
      Number(game.config().trainGold("ally", 0, 1, 1, player)),
      1,
    );
    const result: Array<{
      tile: TileRef;
    ...
    const selfWeight =
      Number(game.config().trainGold("self", 0, 1, 1, player)) / maxTradeGold;
    ...
    for (const neighbor of player.nearby()) {
      ...
      const relType = player.isOnSameTeam(neighbor)
        ...
        : player.isAlliedWith(neighbor)
          ...
          : "other";
      const weight =
        Number(game.config().trainGold(relType, 0, 1, 1, player)) / maxTradeGold;
